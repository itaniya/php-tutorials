...
while($row = mysqli_fetch_assoc($result)){
    ...
    <td>{$row['last_name']}</td>
    <td><button class='btn btn-sm btn-primary edit-btn' data-eid='{$row['roll_no']}' data-toggle='modal' data-target='#model-edit'>Edit</button></td>
    <td><button class='btn btn-sm btn-danger delete-btn' data-id='{$row['roll_no']}'>Delete</button></td>
    </tr>
    ";
}
